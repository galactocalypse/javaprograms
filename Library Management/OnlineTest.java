...
int seconds, minutes;
static int quesnum, itemCheck, mark; 
...
if(start_test){
String temp="";
if(QuestionSeries.n[quesnum]==1) temp="<html>   Choose only ONE Option</html>";
else if(QuestionSeries.n[quesnum]==2) temp="<html>  Choose TWO Options</html>";
else if(QuestionSeries.n[quesnum]==3) temp="<html>  Choose THREE Options</html>";
else temp="<html>  ALL are true true</html>";
choose.setText(temp);
}
else {
...
"There is only "+QuestionSeries.n[quesnum]+" possible answer to question "+(quesnum+1)+
...
"There are only "+QuestionSeries.n[quesnum]+" possible answers to question "+(quesnum+1)+
"<html>","User Information Message",JOptionPane.INFORMATION_MESSAGE);
}
itemCheck=0;
} 
...
outFile.println(studentname+Temp+" "+mark+" out of 25");
